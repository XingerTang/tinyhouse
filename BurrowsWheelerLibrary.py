...
class BurrowsWheelerLibrary:
    ...
    def getHaplotypeMatches(self, haplotype, start, stop):
        nHaps, hapIndexes = getConsistentHaplotypes(
            self.library, haplotype, start, stop
        )
        haps = [
            (self.hapList[hapIndexes[index, 0]], hapIndexes[index, 1])
            for index in range(nHaps)
        ]
        return haps
    ...
    for i in range(stop - 2, start - 1, -1):
        ...
        for interval in range(nIntervals):
            ...
            if hap[i] == 0 or hap[i] == 9:
                ...
                if (
                    upperR > lowerR
                ):  # Needs to be greater than. Regions no longer inclusive.
