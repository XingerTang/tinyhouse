...
class Pedigree(object):
    ...
    def readInAAP(self, fileName):
        ...
        for value in data_list:
            ...
            if len(data) != nLoci:
                print(
                    f"ERROR: For {mfx}, not all loci have an alternative allele frequency in the `-alt_allele_prob_file` input. \nExiting..."
                )
                sys.exit(2)
            if mfx[:3] == "MF_":
                ...
                try:
                    ...
                    print(
                        f"ERROR: For {mfx}, the alternative allele frequency data gave a ValueError. Please check the `-alt_allele_prob_file` input. \nExiting..."
                    )
                    sys.exit(2)
                self.AAP[mfx] = current_aap
            else:
                print(
                    f"ERROR: All metafounders must have the prefix 'MF_'. {mfx} does not but is present in the `-alt_allele_prob_file`. Please remove or rename {mfx}. \nExiting..."
                )
                sys.exit(2)
