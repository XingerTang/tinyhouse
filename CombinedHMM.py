...
class HaploidMarkovModel:
    def __init__(self, n_loci, error, recombination_rate=None):
        ...
        self.apply_sampling = self.create_sampling_algorithm(
            NumbaUtils.multinomial_sample
        )
    ...
    def update_paramaters(self, n_loci, error, recombination_rate=None):
        ...
        if type(recombination_rate) is float:
            self.recombination_rate = np.full(
                n_loci, recombination_rate, dtype=np.float32
            )
        else:
    ...
    def run_HMM(self, point_estimates=None, algorithm="marginalize", **kwargs):
        ...
        if algorithm == "marginalize":
            ...
            genotype_probabilities = self.calculate_genotype_probabilities(
                total_probs, **kwargs
            )
    ...
    def get_point_estimates(
        self, individual, haplotype_library, library_calling_threshold=0.95, **kwargs
    ):
        called_haplotypes = haplotype_library.get_called_haplotypes(
            threshold=library_calling_threshold
        )
        mask = self.get_mask(called_haplotypes)
        point_estimates = self.njit_get_point_estimates(
            individual.genotypes, called_haplotypes, self.error, mask
        )
    ...
    def create_directional_smoothing(self):
        ...
        @jit(nopython=True, nogil=True)
        def directional_smoothing(
            point_estimate, recombination_rate, forward=False, backward=False
        ):
        ...
        def apply_smoothing(point_estimate, recombination_rate):
            ...
            est = (
                ...
                * directional_smoothing(
                    point_estimate, recombination_rate, forward=True
                )
                * directional_smoothing(
                    point_estimate, recombination_rate, backward=True
                )
            )
    ...
    def create_sampling_algorithm(self, selection_function):
        ...
        def sample_path(point_estimate, recombination_rate):
            ...
            forward_and_point_estimate = point_estimate * directional_smoothing(
                point_estimate, recombination_rate, forward=True
            )
            ...
            output[-1].ravel()[
                selected_index
            ] = 1  # Set the output value at the selected_index to 1.
    ...
    def calculate_genotype_probabilities(
        self, total_probs, haplotype_library, **kwargs
    ):
        haplotype_dosages = haplotype_library.get_haplotypes()
        return self.njit_calculate_genotype_probabilities(
            total_probs, haplotype_dosages
        )
    ...
    def njit_calculate_genotype_probabilities(total_probs, reference_haplotypes):
        ...
        geno_probs = np.full(
            (2, n_loci), 0.0000001, dtype=np.float32
        )  # Adding a very small value as a prior incase all of the values are missing.
...
class DiploidMarkovModel(HaploidMarkovModel):
    ...
    def extract_reference_panels(
        ...
        paternal_haplotype_library=None,
        maternal_haplotype_library=None,
    ):
        if (
            maternal_haplotype_library is not None
            and paternal_haplotype_library is not None
        ):
            seperate_reference_panels = True
            return (
                paternal_haplotype_library,
                maternal_haplotype_library,
    ...
    def get_point_estimates(self, individual, library_calling_threshold=0.95, **kwargs):
        (
            ...
            maternal_haplotype_library,
            seperate_reference_panels,
        ) = self.extract_reference_panels(**kwargs)
        ...
        mask = self.get_mask(paternal_called_haplotypes) & self.get_mask(
            maternal_called_haplotypes
        )
        return self.njit_get_point_estimates(
            individual.genotypes,
            paternal_called_haplotypes,
            maternal_called_haplotypes,
            self.error,
            mask,
        )
    ...
    @staticmethod
    @jit(nopython=True, nogil=True)
    def njit_get_point_estimates(
        indGeno, paternalHaplotypes, maternalHaplotypes, error, mask
    ):
    ...
    def calculate_genotype_probabilities(
        ...
        **kwargs,
        ...
        (
            ...
            maternal_haplotype_library,
            seperate_reference_panels,
        ) = self.extract_reference_panels(
            haplotype_library, paternal_haplotype_library, maternal_haplotype_library
        )
        return self.njit_calculate_genotype_probabilities(
            total_probs,
            paternal_haplotype_library.get_haplotypes(),
            maternal_haplotype_library.get_haplotypes(),
            seperate_reference_panels,
        )

    @staticmethod
    @jit(nopython=True, nogil=True)
    def njit_calculate_genotype_probabilities(
        total_probs, paternal_haplotypes, maternal_haplotypes, seperate_reference_panels
    ):
                ...
                for k in range(n_mat):
                    ...
                    if seperate_reference_panels or j != k:
                        ...
                        if pat_value != 9 and mat_value != 9:
                            # Add in a sum of total_probs values.
                            geno_probs[0, i] += (
                                prob_value * (1 - pat_value) * (1 - mat_value)
                            )  # aa
                            geno_probs[1, i] += (
                                prob_value * (1 - pat_value) * mat_value
                            )  # aA
                            geno_probs[2, i] += (
                                prob_value * pat_value * (1 - mat_value)
                            )  # Aa
    ...
    def transmission(
        ...
        ) ** 2  # No recombination on either chromosome.
        output[:] += (
            np.expand_dims(row_sums, 0)
            / output.shape[0]
            * recombination_rate
            * (1 - recombination_rate)
        )  # recombination on the maternal (second) chromosome)
        output[:] += (
            np.expand_dims(col_sums, 1)
            / output.shape[1]
            * recombination_rate
            * (1 - recombination_rate)
        )  # recombination on the paternal (first) chromosome)
    ...
    def maximum_likelihood_step(
        ...
        for i in range(n_pat):
            ...
            paternal_max_value[i] = (
                previous_path_score[i, index]
                * (1 - recombination_rate)
                * recombination_rate
                / n_mat
            )
        ...
        for j in range(n_mat):
            ...
            maternal_max_value[j] = (
                previous_path_score[index, j]
                * (1 - recombination_rate)
                * recombination_rate
                / n_pat
            )
...
class JointMarkovModel(HaploidMarkovModel):
    ...
    def transmission(
        ...
        diploid_section[:] += (
            np.expand_dims(row_sums, 0)
            / diploid_section.shape[0]
            * recombination_rate
            * (1 - recombination_rate)
        )  # recombination on the maternal (second) chromosome)
        diploid_section[:] += (
            np.expand_dims(col_sums, 1)
            / diploid_section.shape[1]
            * recombination_rate
            * (1 - recombination_rate)
        )  # recombination on the paternal (first) chromosome)
        diploid_section[:] += (
            diploid_weight * recombination_rate**2 / diploid_section.size
